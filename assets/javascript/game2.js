...
// How it works - DONE
  ...
  //         Computer updates display of 'Letters already guesses'                                    - DONE
  //         AND Computer decriments the guess counter                                                - DONE
  ...
  //    1. If true, 
  //        1. Add a win and update 'Wins"                                                            - DONE
  //        2. Go to next round unless it is last round in which case game is over                    - DONE
  //    2. Otherwise, do nothing and wait                                                           
  // 10. < EVALUATE > Is the "Number of guesses remaining 0"?                                         - DONE
  //    1. If True
  //        1. Go to Next Round unless it is last round in which case game is over                    - DONE
  //    2. Otherwise, do nothing and wait

// Caveats
// Try to do it without using a page refresh 
// Comments: Struggled with initialization and stopping key events when game was over

// Notes
// followed structure from another hangman github file after looking through several
// only looked if i hit dead ends - changed things that were advanced uneeded or barrier 
...
var game = {
  newGame: function () {
    ...
    ///////// SET/REST VARIABLES /////////
    // round counter used to determine if at end of game
    roundCounter++; 
    ...
    // displayImage.innerHTML = "<img src=\"" + currentHangman.image + "\"> " + "<audio controls class=\"hidesong\" src=\"" +currentHangman.song + "\" autoplay>";
  ...
  update_Game: function() {
    // GAME OVER - LOST LAST ROUND
    if ( incorrectGuessCount === 0 && roundCounter === 2 ) {
      document.removeEventListener('keyup', game.gameLoop);
      // currentHangman.song.sound.pause();
      game.display_gameOver();
    } 
    // GAME OVER - WON LAST ROUND
    else if ( currentHangmanAnswer.indexOf("_") === -1  && roundCounter === 2 ) {
      document.removeEventListener('keyup', game.gameLoop);
      // currentHangman.song.sound.pause();
      winsCounter++;
      game.display_gameOver();
    ...
    // USER LOST
    else if (incorrectGuessCount === 0 && roundCounter < 2) {
  ...
  update_HangmanAnswer: function(guess) {
    ...
    //   FIRST LINE
    //   clears the array each guess and contains only one item at a time
    currentHangmanAnswerIndex = [];
  ...
  display_gameOver: function() {

    displayStart.innerHTML = "GAME OVER";
    displayScore.innerHTML = winsCounter;
    displayImage.innerHTML = " ";
    displayInstructions.innerHTML = " ";
    displayCurrentHangmanAnswer.innerHTML = " ";
    displayIncorrectGuessCount.innerHTML = " ";
    displayIncorrectLetters.innerHTML = " ";
    
  },

// MAIN LOOP FUNCTION ///////////////////////////
// put the game in a loop function and attach to document event listener
// by doing this, can remove the event listener and kill input after last round
  gameLoop: function (event) {
    //check if we remove event listener with this when the game is over
    console.log("i fired");
    
    var userGuess = event.key;

    // runs on every keypress, but is true only once 
    ...
    // check if ran out guesses, finsihed, and/or if game is over
    // updates rest of the fields inline
    game.update_Game();
  }
...
//////////////////////////////////////////////////////////////
//initialize - this also runs inside main loop anytime new round starts
game.newGame();

// Main loop function
document.addEventListener('keyup', game.gameLoop);
